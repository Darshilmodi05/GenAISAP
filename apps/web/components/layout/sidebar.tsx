...
export const Sidebar = () => {
  ...
  return (
    <aside
      className={cn(
        "relative h-screen bg-[#0a0a0f] border-r border-white/5 transition-all duration-700 ease-[cubic-bezier(0.4,0,0.2,1)] flex flex-col z-50 overflow-hidden",
    ...
    >
      ...
      {/* Brand Section */}
      <div className="h-[140px] flex items-center justify-center border-b border-white/5 relative">
        <Link href="/dashboard/home" className="flex items-center gap-5 group/logo">
           <motion.div 
            whileHover={{ rotate: 180, scale: 1.1 }}
            transition={{ type: 'spring', damping: 10 }}
            className="w-14 h-14 bg-gradient-to-br from-primary to-violet-800 flex items-center justify-center rounded-2xl shadow-[0_0_30px_rgba(124,58,237,0.4)]"
           >
           ...
           {isSidebarOpen && (
             ...
             >
                <span className="text-xl font-black tracking-tighter text-white leading-none">GenAISAP</span>
            ...
            >
              ...
              {item.badge && isSidebarOpen && (
                <span className={cn(
                  ...
                  active ? "bg-black/10 text-black" : "bg-primary/10 text-primary"
                )}>
                  {item.badge}
                </span>
              )}
      ...
      <div className="p-8 border-t border-white/5 space-y-6">
        {isSidebarOpen && (
          <motion.div 
            ...
            className="flex items-center gap-5 p-5 bg-white/5 rounded-[2.5rem] border border-white/5 group hover:border-primary/30 transition-all cursor-pointer"
          >
             <Avatar initials="DM" size="md" status="active" className="rounded-2xl border-white/10" />
             <div className="flex flex-col min-w-0">
                <span className="text-sm font-black text-white truncate">DARSHIL MODI</span>
        ...
        <button 
          ...
          className="w-full h-14 flex items-center justify-center bg-white/5 hover:bg-white hover:text-black transition-all rounded-[2rem] shadow-2xl group"
        >
